...
A lightweight **FastAPI** service providing free market data powered by **Alpha Vantage**
(price quotes, candles) and **Google News RSS** (headlines), with the **ta** library for
technical indicators computed locally and **VADER sentiment analysis** on news headlines.

> **Alpha Vantage free tier:** 25 API requests/day · 5 requests/minute.
> Each ticker analysis uses exactly **2 calls** (1 quote + 1 daily prices).
> All 11 technical indicators are computed locally — no extra API calls.
> A 5-minute in-process cache ensures `candles` and `indicators` share one daily-prices call.
...
|----------|----------|-------------|
| `ALPHA_VANTAGE_API_KEY` | **Yes** | Free key from [alphavantage.co](https://www.alphavantage.co/support/#api-key) |
| `API_KEY` | No | Protects your endpoints — all requests must include `?api_key=VALUE` |
| `PORT` | No | Defaults to 8000 locally |
...
In the Render dashboard, open your service → **Environment** tab, then add:

| Key | Value |
|-----|-------|
| `ALPHA_VANTAGE_API_KEY` | your Alpha Vantage key (required) |
| `API_KEY` | your endpoint protection key (optional) |

Render restarts the service automatically after saving.
...
Pass `ALPHA_VANTAGE_API_KEY` (required) and optionally `API_KEY` (endpoint protection):
...
- **Alpha Vantage free tier:** 25 requests/day, 5 requests/minute. Each ticker
  analysis uses exactly 2 calls (1 `GLOBAL_QUOTE` + 1 `TIME_SERIES_DAILY`), giving
  you ~12 tickers per day on the free plan.
- **Caching:** daily price data is cached in-process for 5 minutes. When the same
  ticker is requested for both `/candles` and `/indicators` in quick succession,
  only 1 API call is made instead of 2.
- **Indicator warm-up:** `TIME_SERIES_DAILY` compact mode returns the last 100
  trading days — sufficient for SMA-50, ADX-14, RSI-14, and all other indicators.
- **News:** headlines are fetched from Google News RSS via `feedparser` — no API
  key required and no rate limits.
- All 11 technical indicators are computed locally using the `ta` library.
  Alpha Vantage's built-in indicator endpoints are never called.
